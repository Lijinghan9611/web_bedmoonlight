...
layui.define(['jquery', 'element', 'nprogress', 'utils'], function(exports) {
    ...
    var Tab = function() {
        this.config = {
            elem: undefined,
            mainUrl: 'desktop.html',   //主页面,固定tab
            renderType: 'iframe',
            openWait: false
        };
        this.v = '1.0.5';
        ...
        tabAdd: function(options) {
            ...
            if (_config.onSwitch) {
                element.on('tab(' + that._filter + ')', function (data) {
                    //var id = that._title.children('li').eq(data.index).attr('lay-id');
                    //if (id == '203') {
                    //    var btn = this.window.document.getElementById("btn_search")
                    //    btn.click();
                    //}
